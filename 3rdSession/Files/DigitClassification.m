...
% display("Accuracy with deepnet without fine tune", string(classAcc));
% 
% 
...
% display("Accuracy with deepnet without fine tune", string(classAcc));
% classAcc=100*(1-confusion(tTest,y));
% display("Accuracy with fine tuned", string(classAcc));
% view(deepnet)
...
plotconfusion(tTest,y);
classAcc=100*(1-confusion(tTest,y));
display("Accuracy with deep neural network 1", string(classAcc));
view(net)

%Compare with normal neural network (2 hidden layers)
net = patternnet([100,50,50]);
net=train(net,xTrain,tTrain);
y=net(xTest);
plotconfusion(tTest,y);
classAcc=100*(1-confusion(tTest,y));
display("Accuracy with deep neural network 2", string(classAcc));
view(net)